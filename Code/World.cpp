...
void WorldManager::RenderWorld(int32 *ambientDrawCount, int32 *lightDrawCount, int32 *lightSourceCount)
{
	...
	if (cameraNode)
	{
		...
		if (fogNode)
		{
			...
			params->fogPlane = !fogNode->GetFogPlane();
			params->fogColor = (1.0F, 1.0F, 1.0F, 0.7F);
			params->fogParams.x = 0.005F; // Fog density

			if (Dot(params->fogPlane, params->cameraPosition) < 0.0) { // calculates m 
				params->fogParams.y = 1.0F;
				
			}
			else{
				params->fogParams.y = 0.0F;
				params->fogParams.w = 1.0F;// set w to positive if greater than 0
			}


			if ((Dot(params->fogPlane, params->cameraPosition) < 0.0)) // if dot(f,c) is 0
				params->fogParams.w = -1.0F; // set w to negative if less than 0
			else if ((Dot(params->fogPlane, params->cameraPosition) > 0.0))
				params->fogParams.w = 1.0F; // set w to positive if more than 0
			else if((Dot(params->fogPlane, params->cameraPosition) == 0.0)) params->fogParams.w = 0.0F; // eqaul to 0

			params->fogParams.z = Dot(params->fogPlane, params->cameraPosition);
...
void WorldManager::BuildWorld(void)
{
	...
	fogNode->nodeTransform.SetTranslation(Point3D(0.0F, 0.0F, 10.0F));
	rootNode->AppendSubnode(fogNode);
